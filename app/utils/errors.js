...
class ErrorManager {
  ...
  messageDisplayingMode = notification => {
    ...
    if (environment === 'development') {
      ...
      if (code && code >= 500) {
        ...
        return {
          header: I18n.t('badRequest'),
          errorMessage: I18n.t('unauthorizedRequest')
        };
      } else {
        return {
  ...
  _notifySubscribers(errorHeader, error, isLocal) {
    const { header = '', errorMessage = '' } = this.messageDisplayingMode({
      errorHeader,
      error
    });

    this.subscribers.forEach(subscriber =>
      subscriber(header, errorMessage, isLocal)
    );
  }
}
export { ErrorManager };
